...
import { QuickResponseCategoryService } from './../../../../core/services/quick-response-category.service';
import {
  Component,
...
} from '@models/QuickResponseCategory.model';
...
export class QuickresponseDashboardComponent implements OnInit {
  ...
  getCategoryName() {
    const element = this.quickResponseCategories().find(
      (item) => item.quickResponseCategoryId == this.activeCategoryFilter()
    );
    return element?.name;
  }
  getActive() {
  ...
  openEdit(item: QuickResponseItem) {
    this.header.set('Editar Respuesta Rápida');
    this.showEdit = true;
    this.itemEdit = item;
  }
  openCreate() {
    this.header.set('Nueva Respuesta Rápida');
