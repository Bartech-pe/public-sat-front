...
export class ChannelScheduleComponent {
  ...
  assignSchedule() {
    if (this.scheduleForm.valid) {
      const payload = {
        ...
        channelId: this.scheduleForm.value.supervisor,
        days: this.scheduleForm.value.days.filter(
          (day: any) => day.startTime || day.endTime
        ),
      };
      this.http
        .post('http://localhost:3001/v1/channel-schedule', payload)
        .subscribe(
  ...
  saveConfig() {
    if (this.configForm.valid) {
      this.http
        .post('/api/configure-outofhours', this.configForm.value)
        .subscribe(
