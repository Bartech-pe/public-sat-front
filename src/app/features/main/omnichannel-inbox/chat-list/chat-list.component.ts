...
import {
  AdvisorChangedDto,
  BotStatusChangedDto,
  ChannelAttentionStatusReverseTag,
...
export class ChatListComponent implements OnInit, OnDestroy {
    ...
    if (
      ...
      this.channelRoomSocketService
        ...
        .subscribe((payload) => {
          ...
          this.chatListInbox = this.chatListInbox
            .map((x) => {
              if (
                x.channelRoomId === payload.channelRoomId &&
                x.attention.id === payload.assistanceId
              ) {
      ...
      this.channelRoomSocketService
        ...
        .subscribe((message: ChannelRoomViewStatusDto) => {
          if (this.chatListInbox.length) {
            const index = this.chatListInbox.findIndex(
              (c) => c.channelRoomId === message.channelRoomId
            );
            this.chatListInbox[index] = {
              ...this.chatListInbox[index],
              unreadCount:
                this.chatListInbox[index].unreadCount - message.readCount,
              lastMessage: {
      ...
      this.channelRoomSocketService
        ...
        .subscribe((message: ChannelRoomNewMessageDto) => {
          if (
            !this.availableChannels.includes(message.channel) &&
            !['administrador', 'supervisor'].includes(
              this.authStore.user()?.role?.name ?? ''
            )
          )
      ...
      this.channelRoomSocketService
        .onAttentionDetailModified()
        .pipe(takeUntil(this.destroy$))
        .subscribe((message: ChannelRoomAssistance) => {
          const index = this.chatListInbox.findIndex(
            (c) => c.attention.id === message.assistanceId
          );
          this.chatListInbox[index] = {
            ...this.chatListInbox[index],
            attention: {
              ...this.chatListInbox[index]?.attention,
      ...
      this.channelRoomSocketService
        ...
        .subscribe((message: AdvisorChangedDto) => {
          const hasChannelRoomWithAdvisorChanged = this.chatListInbox.some(
            (x) => x.channelRoomId == message.channelRoomId
          );
      ...
      this.channelRoomSocketService
        ...
        .subscribe((payload: ChannelRoomAssistance) => {
          const index = this.chatListInbox.findIndex(
            (c) =>
              c.channelRoomId === payload.channelRoomId &&
              c.attention.id === payload.assistanceId
          );
          if (
            this.authStore.user()?.id === payload.userId ||
            ['supervisor', 'administrador'].includes(
              this.authStore.user()?.role?.name ?? ''
            )
          ) {
  ...
  updateChatList(message: ChannelRoomNewMessageDto) {
    try {
      ...
      const index = this.chatListInbox.findIndex(
        (c) =>
          c.channelRoomId == message.channelRoomId &&
          ...
          (chat.lastMessage = {
            citizen: {
              ...
              name:
                ...
                message.message.sender.alias ||
                message.message.sender.phone,
            ...
            (message.advisor?.id != null &&
              (['unread', 'all'].includes(this.selectedFilter?.value ?? '') ||
                this.selectedFilter?.value ===
                  this.getChannelAttentionStatusReverseTag(
                    message.attention.status
                  ))))
        ) {
  ...
  getChannelAttentionStatusReverseTag(
    status: ChatListInbox['attention']['status']
  ): string {
  ...
  async getCurrrentStatusByUser(channel: string): Promise<void> {
    try {
      ...
      const statusLabel = this.statusOptions.find(
        (x: MenuItem) =>
          x.id === response.data?.userStatus ||
          x.label === response.data?.userStatus
      );
  ...
  async getUserStatusesByChannel(channel: string): Promise<void> {
    try {
      ...
      if (response?.data && response.data.length > 0) {
        this.statusOptions = response.data.map(
  ...
  async getAvailableChannelsByUser() {
    try {
      const response = await firstValueFrom(
        this.inboxService.getAvailableChannelsByUser()
      );
  ...
  changeUserStatus(
    ...
    this.inboxService
      ...
      .subscribe((response) => {
        if (response.success) {
          this.msg.success(
            'Su estado ha sido actualizado correctamente.',
            'Estado del asesor',
            5000
          );
          ...
          if (response?.error) {
            ...
            this.msg.error(
              'Su estado no ha podido ser actualizado, por favor contacte a soporte',
              'Estado del asesor',
              5000
            );
            return;
          ...
          this.msg.warn(
            'Su estado no ha podido ser actualizado',
            'Estado del asesor',
            5000
          );
        }
